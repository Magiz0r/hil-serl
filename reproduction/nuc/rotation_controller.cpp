...
bool ResponsiveCartesianImpedanceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 20, &ResponsiveCartesianImpedanceController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("ResponsiveCartesianImpedanceController: Could not read parameter arm_id");
    return false;
  }
  std::vector<std::string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "ResponsiveCartesianImpedanceController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "ResponsiveCartesianImpedanceController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "ResponsiveCartesianImpedanceController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "ResponsiveCartesianImpedanceController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "ResponsiveCartesianImpedanceController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "ResponsiveCartesianImpedanceController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "ResponsiveCartesianImpedanceController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&ResponsiveCartesianImpedanceController::complianceParamCallback, this, _1, _2));
...
void ResponsiveCartesianImpedanceController::starting(const ros::Time& /*time*/) {
...
void ResponsiveCartesianImpedanceController::update(const ros::Time& time,
  ...
  orientation_d_ = orientation_d_.slerp(0.02, orientation_d_target_);
  Ki_ = filter_params_ * Ki_target_ + (1.0 - filter_params_) * Ki_;
}

void ResponsiveCartesianImpedanceController::publishZeroJacobian(const ros::Time& time) {
...
Eigen::Matrix<double, 7, 1> ResponsiveCartesianImpedanceController::saturateTorqueRate(
...
void ResponsiveCartesianImpedanceController::complianceParamCallback(
...
void ResponsiveCartesianImpedanceController::equilibriumPoseCallback(
...
PLUGINLIB_EXPORT_CLASS(serl_franka_controllers::ResponsiveCartesianImpedanceController,

...
def main():
    ...
    if manual:
        ...
        if official:
            outcome.update(response_profile="ram_measured_step", target_speed_mm_s=None,
                           angular_speed_deg_s=None, action_hz=1 / ACTION_PERIOD,
                           position_action_scale_mm=1000 * POSITION_ACTION_SCALE,
                           rotation_action_scale_rad=ROTATION_ACTION_SCALE,
                           predicted_joint_speed_limit_rad_s=ACTION_JOINT_SPEED)
        if free:
            outcome.update(workspace_radius_mm=None, orientation_radius_deg=None,
                           session_seconds=None, idle_seconds=None)
    controller = subscriber = publisher = None
            ...
            with (run / "samples.jsonl").open("x") as samples:
                while free or time.monotonic() < (session_deadline if manual else window.deadline):
                    ...
                    if packet["seq"] != last_seq:
                        if manual:
                            target, quat = guard.step(packet, now, now - last_step)
                            if packet["enable"]:
                        ...
                        if target != sent_target or quat != sent_quat:
                            msg = PoseStamped()

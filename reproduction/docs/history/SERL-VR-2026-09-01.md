...
> 旧项目参考手册：其中服务、路径和状态不属于本轮 HIL-SERL 部署。三处指向旧项目的相对链接已改为路径文字，内容保留用于追溯。当前入口见 [复现总览](../../README.md)。
...
`serl_franka_controllers-soft-bringup-defaults.patch`（旧项目路径：`../patches/serl_franka_controllers-soft-bringup-defaults.patch`）:
...
`deploy/serl`（旧项目路径：`../deploy/serl/README.md`）; it applies
the local initialization patch（旧项目路径：`../patches/serl_franka_controllers-initialize-integrator.patch`）
